...
if [ -S "$HOME/.mpv-socket" ]; then
    if command -v socat &>/dev/null; then
        _nc_probe() { socat - UNIX-CONNECT:"$HOME/.mpv-socket",connect-timeout=1; }
    elif nc -h 2>&1 | grep -q '\-N'; then
        _nc_probe() { nc -N -U -w 1 "$HOME/.mpv-socket"; }
    else
        _nc_probe() { ncat -U -w 1 "$HOME/.mpv-socket"; }
    fi
    if ! (echo '{}' | _nc_probe >/dev/null 2>&1); then
        ...
        -auto)
            if [ -f "$HOME/.cache/mpv/auto_enabled" ]; then
                ...
                if [ "$MPV_RUNNING" = true ]; then
                    echo '{ "command": ["set_property", "loop-file", "no"] }' | mpv_nc 1 > /dev/null
                    echo '{ "command": ["set_property", "loop-playlist", "no"] }' | mpv_nc 1 > /dev/null
                fi
...
if [ -z "$1" ]; then
    ...
    # Get initial status
    mapfile -t init < <(echo -e '{"command":["get_property","playlist-count"]}\n{"command":["get_property","shuffle"]}\n{"command":["get_property","loop-file"]}\n{"command":["get_property","loop-playlist"]}\n{"command":["get_property","af"]}' | mpv_nc 1 2>/dev/null | jq -s -r 'map(select(.event == null)) | .[0].data // 0, .[1].data // false, .[2].data // "no", .[3].data // "no", (if .[4].data == null or .[4].data == [] then "off" else "on" end)')    
    ...
    if [ -n "$selection" ]; then
        ...
        track_info=$(echo '{ "command": ["get_property", "playlist"] }' | mpv_nc 1 2>/dev/null)
        ...
        if [ "$count" -gt 1 ] || [ -f "$SELECTION_MODE_FILE" ]; then
             ...
             if echo "$action" | grep -q "Remove"; then
                 ...
                 if [ "$was_playing" = true ]; then
                     is_paused=$(echo '{ "command": ["get_property", "pause"] }' | mpv_nc 1 2>/dev/null | jq -r '.data // "false"')
