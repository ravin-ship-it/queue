show_queue() {
    ...
    if [ "$MPV_RUNNING" = false ]; then
        for i in {1..10}; do
            if [ -S "$SOCKET" ] && echo '{ "command": ["get_property", "playlist-count"] }' | mpv_nc 1 &>/dev/null; then
                MPV_RUNNING=true
                break
            fi
            sleep 0.2
    ...
    PLAYLIST_JSON=$(echo "{ \"command\": [\"get_property\", \"playlist\"] }" | mpv_nc 1 2>/dev/null)
...
cmd_remove() {
    local track_info=$(echo '{ "command": ["get_property", "playlist"] }' | mpv_nc 1)
    ...
    if [ ${#INDICES_TO_REMOVE[@]} -gt 0 ]; then
        ...
        for idx in $sorted_indices; do
            ...
            echo "{ \"command\": [\"playlist-remove\", $((idx - 1))] }" | mpv_nc 1 > /dev/null
        ...
        if [ "$was_playing_removed" = true ]; then
            local is_paused=$(echo '{ "command": ["get_property", "pause"] }' | mpv_nc 1 2>/dev/null | jq -r '.data // "false"')
...
cmd_move() {
    ...
    echo "{ \"command\": [\"playlist-move\", $((from - 1)), $target_idx] }" | mpv_nc 1 > /dev/null
...
cmd_shuffle() {
    ...
    if [ "$mode" == "list" ] || [ "$mode" == "all" ]; then
        echo '{"command": ["playlist-shuffle"]}' | mpv_nc 1 > /dev/null
        echo -e "${C_PINK}🔀 Playlist entries shuffled.${C_RESET}"
        save_current_playlist true >/dev/null 2>&1 & disown
    else
        # Toggle shuffle property
        local current=$(echo '{ "command": ["get_property", "shuffle"] }' | mpv_nc 1 2>/dev/null | jq -r '.data // "false"')
        if [ "$current" == "true" ]; then
            echo '{ "command": ["set_property", "shuffle", false] }' | mpv_nc 1 > /dev/null
            echo -e "${C_ORANGE}🔀 Shuffle Mode: ${C_WHITE}OFF${C_RESET}"
        else
            echo '{ "command": ["set_property", "shuffle", true] }' | mpv_nc 1 > /dev/null
...
cmd_remove_redundant() {
    ...
    if [ -n "$playlist_file" ]; then
        ...
        for idx in $indices_to_remove;
         do
            echo "{ \"command\": [\"playlist-remove\", $idx] }" | mpv_nc 1 > /dev/null
        done
...
auto_queue_related() {
    ...
    if [ -z "$input_filename" ] && [ "$count" -gt 0 ]; then
        ...
        local seed_json=$(echo "{\"command\":[\"get_property\", \"playlist/$target_idx\"]}" | mpv_nc 1 2>/dev/null | jq -r '.data // empty')
    ...
    # --- 3. Deduplication ---
    local pl_json=$(echo '{"command":["get_property","playlist"]}' | mpv_nc 2 2>/dev/null)
